"""
Диаризация — определяем, кто из говорящих сказал каждую реплику.

Класс SpeakerDiarizer работает только с одноканальной (моно) записью.
Он берёт каждый кусочек речи, превращает голос в набор чисел (отпечаток
голоса) и группирует похожие отпечатки. Один человек -> один отпечаток ->
один спикер.

Для телефонных записей с двумя каналами это не нужно: там и так понятно,
что один канал = один человек. Этим занимается оркестратор, а не этот класс.
"""
import numpy as np
